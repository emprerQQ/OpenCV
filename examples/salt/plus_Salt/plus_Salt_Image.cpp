...
#define NAME_cpp " plus_Salt_Image "
...
//		channel : 颜色通道号  0  1  2
void salt(Mat &image, Mat &result, int n)
{
	int k;
	int i;
	int j;
	
	result = image.clone();//我们不希望原始图像被改变，因此创建一份输入图像的拷贝。最简单的创建一个图像的“深拷贝”的方式是调用clone函数
	for(k=0; k<n; k++)
	{
		//rand() 是随机数生成函数
		i = rand()%result.cols;  //公有成员变量 cols(列) 和 rows(行)给出图像的宽和高
		j = rand()%result.rows;
...
int main( int argc, char** argv )
{
	...
	//椒盐噪点处理
	salt( image, plus_Salt_Image, 2000 );         // 1: input image
						 // 2: output image
						 // 3: number
...
//	imwrite( "./"NAME_cpp".jpg", Salt_Image );
	...
	namedWindow( "plus_Salt_Image", CV_WINDOW_AUTOSIZE );

	imshow( imageName, image );// Show our image inside it
	imshow( "plus_Salt_Image", plus_Salt_Image );
